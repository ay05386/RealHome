...
class propertyRemoteMediator @Inject constructor(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Property>): MediatorResult {
        return try {
    ...
    private suspend fun getRemoteKeyClosetToCurrentPosition(state: PagingState<Int, Property>):
            propertyRemoteKey? {
    return state.anchorPosition?.let{position -> state.closestItemToPosition(position)?.id?.let {id ->propertyRemoteKeyDao.getRemoteKey(id=id)
    }
    }
    }
    ...
    private suspend fun getRemoteKeyForFirst(state: PagingState<Int, Property>): propertyRemoteKey? {
        return state.pages.firstOrNull{it.data.isNotEmpty()}?.data?.firstOrNull()?.let {
            property -> propertyRemoteKeyDao.getRemoteKey(id = property.id)
        }
    }
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Property>): propertyRemoteKey? {
        return state.pages.lastOrNull{it.data.isNotEmpty()}?.data?.lastOrNull()?.let { property -> propertyRemoteKeyDao.getRemoteKey(id = property.id) }
    }
